 ...
 * Falls back to a generated identicon if no Gravatar is found.
...
export function getGravatarUrl(email: string, size: number = 80): string {
  ...
  // d=mp shows a generic person silhouette if no Gravatar
  // d=identicon shows a geometric pattern
  // d=retro shows an 8-bit style avatar
  return `https://www.gravatar.com/avatar/${hash}?s=${size}&d=retro&r=g`;
}

// ─── Tiny MD5 implementation (RFC 1321) ─────────────────────────────────────
...
function md5(str: string): string {
  ...
  function md5blks(s: string) {
    const md5blksLen = (((s.length + 8) >>> 6) + 1) * 16;
    const md5blk = new Array<number>(md5blksLen).fill(0);
    for (let i = 0; i < s.length; i++) {
      md5blk[i >> 2] |= s.charCodeAt(i) << ((i % 4) * 8);
    }
    md5blk[s.length >> 2] |= 0x80 << ((s.length % 4) * 8);
    md5blk[md5blksLen - 2] = s.length * 8;
    return md5blk;
  }
